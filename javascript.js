...
function AddCategory() {
	//add some way to input and then send it as a variable down below
	counter++;
	$("<li class='budg-item'> New Category <input type='range' class='slider' min='0' max='50' value='0' step='10' oninput='showValue" + 
		counter + "(this.value)' /><span id='range" + counter + "'>0</span></li>").appendTo("#budg-list");

	$("<script> function showValue" + counter + "(newValue)" +
		"{ document.getElementById('range" + counter + "').innerHTML=newValue; } </script>").appendTo("body");
}
